import { allTask, executeTask, newTask } from "@/API/task-state"
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query';
import { hanldeErrors } from '../helpers/erros';
import Swal from 'sweetalert2/dist/sweetalert2.js'
import { updateTask } from '../API/task-state';
import { toast } from "react-toastify";


export const useTasks = () => {
    return useQuery(["task-state", "list"], allTask,{
        staleTime: Infinity,
...
export const useNewTask = () => {
    ...
    return useMutation( newTask,{
        onSuccess: (response, task) => {
            queryClient.invalidateQueries(["task-state", "list"]);
            Swal.fire({
                icon: 'success',
