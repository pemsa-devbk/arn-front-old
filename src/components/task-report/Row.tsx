...
import { useExecuteTask } from '../../hooks/task-report';
...
interface Props {
    task: TaskReport;
    openModalEdit: (task: TaskReport) => void;
    openModalView: (task: TaskReport) => void;
    openModalContact: (task: TaskReport) => void;
...
export const Row = ({ task, openModalEdit, openModalView, openModalContact }: Props) => {
    ...
    const { mutate: startMail, isLoading: isLSTAM } = useStartMail('task-report');
    const { mutate: stopMail, isLoading: isLSTOM } = useStopMail('task-report');
    const { mutate: startWhatsApp, isLoading: isLSTAW } = useStartWhatsApp('task-report');
    const { mutate: stopWhatsApp, isLoading: isLSTOW } = useStopWhatsApp('task-report');
    const { mutate: startTelegram, isLoading: isLSTAT } = useStartTelegram('task-report');
    const { mutate: stopTelegram, isLoading: isLSTOT } = useStopTelegram('task-report');
    const { mutate: startTask, isLoading: isLSTAK } = useStartTask('task-report');
    const { mutate: stopTask, isLoading: isLSTOK } = useStopTask('task-report');
    const { mutate: deleteTask, isLoading: isLDel } = useDeleteTask('task-report');
        ...
        <>
            <tr className='table__col-primary'>
                ...
                <td className='table-text'>
                    ...
                    {
                        filters?.find(filter => filter.idFilter === task.idFilter)?.name || 'No econtrado'
                    }
                </td>
                <td className='table-text'>{task.lastRun ? new Date(task.lastRun).toLocaleString('es-ES', { timeZone: 'America/Chihuahua'}) : 'Sin ejecutar'}</td>
